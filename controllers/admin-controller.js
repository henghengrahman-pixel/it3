...
export async function dashboardPage(req,res){
  const [slides, quickActions, settings, posts, ads] = await Promise.all([getSlides(), getQuickActions(), getSettings(), getPosts({ includeDrafts:true }), getAds()]);
  res.render('admin/dashboard',{layout:'layouts/admin',pageTitle:'Dashboard Admin',slides,quickActions,settings,posts,ads});
}
